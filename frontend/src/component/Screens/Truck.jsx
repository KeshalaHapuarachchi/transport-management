...
function Truck() {
  ...
  useEffect(() => {
    (async () =>
      await axios.get("http://localhost:5000/deliverVehicle/").then((res) => {
        setData(res.data.filter((veh) => veh.vehicleType === "Truck"));
        console.log(Data);
        setReload(false);
      }))();
  }, [reload]);
  ...
  const genPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text("Truck Details", 10, 10);

    doc.autoTable({
      html: "#content",
    });
    doc.save("Truck Details.pdf");
  };

  return (
    <div className="lorry">
      <div className="title">
        <h1>Available Truck List</h1>
      ...
      <div className="searchinput">
        <Input
          ...
          name="searchitem"
        />
        <Button onClick={genPDF}>Download All Truck Details</Button>
      </div>
