...
function Van() {
  ...
  useEffect(() => {
    (async () =>
      await axios.get("http://localhost:5000/deliverVehicle/").then((res) => {
        setData(res.data.filter((veh) => veh.vehicleType === "Van"));
        console.log(Data);
        setReload(false);
      }))();
  }, [reload]);
  ...
  const genPDF = () => {
    ...
    doc.setFontSize(20);
    doc.text("Van Details", 10, 10);

    doc.autoTable({
      html: "#content",
    });
    doc.save("Van Details.pdf");
  };

  return (
    <div className="lorry">
      <div className="title">
        <h1>Available Van List</h1>
      ...
      <div className="searchinput">
        <Input
          ...
          name="searchitem"
        />
        <Button onClick={genPDF}>Download All Van Details</Button>
      </div>
